...
void bfs(vector<int> initialState, vector<int> targetState) {
    ...
    int maxNodesToProcess = processQueue.size();

    while(!processQueue.empty() && processQueue.front().depth <= 30) {
        ...
        if (vecsEqual(currentFrontNode.state, targetState)) {
            ...
            printNode(currentFrontNode);
            cout << "Number of nodes visited: " << visitedStates.size() << endl;
            cout << "Max size of list: " << maxNodesToProcess << endl;
            break;
        } else {
            ...
            if (processQueue.size() > maxNodesToProcess) {
                maxNodesToProcess = processQueue.size();
            }
        }
        processQueue.pop();
    }
    if (processQueue.empty() || processQueue.front().depth >= 30) {
...
void dfs(vector<int> initialState, vector<int> targetState) {
    ...
    vector<vector<int> > visitedStates;
    int maxNodesToProcess = processStack.size();

    while(!processStack.empty() && processStack.top().depth <= 10000) {
        ...
        if (vecsEqual(currentFrontNode.state, targetState)) {
            ...
            if (processStack.size() > maxNodesToProcess) {
                maxNodesToProcess = processStack.size();
            }
        }
    }
    if (processStack.empty() || processStack.top().depth >= 10000) {
        cout << "Never Found a solution..." << endl;
        cout << "Number of nodes visited: " << visitedStates.size() << endl;
        cout << "Max size of list: " << maxNodesToProcess << endl;
    }
...
void ids(vector<int> initialState, vector<int> targetState) {
    ...
    int maxDepth = 0;
    while (!foundSolution && maxDepth <= 21) {
